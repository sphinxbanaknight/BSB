...
def pinger():
    ...
    ignlist = [item for item in rostersheet.col_values(3) if
               item and item != 'IGN' and item != 'READ THE NOTES AT [README]']
    idlist = [item for item in fullofsheet.col_values(3) if
              item and item != 'UNIQUE:' and item != 'Discord Tag' and item != 'READ THE NOTES AT [README]']
    row = 3
    dsctag = []
    dscid = []
...
async def on_ready():
    ...
    while True:
        ...
        if not isarchived and ph_time_formated in tf_archive:
            isarchived = True
            await botinitsk.send(
                '```Automatically cleared the roster! Please use /att y/n, y/n again to register your attendance.```')
        ...
        # Timed event [auto-reminder]: a soft reminder message into #announcement. Remove on next event
            ...
            if debugger: await botinitsk.send(
                f'{feedback_debug} {ph_time_formated} Reminder1 isreminded1={isreminded1} START')
            isreminded1 = True
            try:
                ...
                if ph_time_formated == t_silk4_archive:
                    ...
                    att_igns = [item for item in rostersheet.col_values(7) if
                                item and item != 'IGN' and item != 'Next WOE:']
                    nratt = len(att_igns)
...
For those who haven't: {feedback_noangrypingplz}'''
            ...
            if debugger: await botinitsk.send(
                f'{feedback_debug} {ph_time_formated} Reminder1 isreminded1={isreminded1} END')
            continue
        # Timed event [auto-reminder]: @mention per player who enlisted but not yet confirmed attendance
            ...
            try:
                try:  # msg1 may not be found
                    ...
                    try:  # find value saved in sheet
                        ...
                        await botinitsk.send(
                            f'Error: `{e}`. Unable to find and delete message based on `{msgid}`. Please manually delete it.')
                except Exception as e:
                ...
                if debugger:  # send to test server if on debugmode
                    ...
                    if taglist != '':
                        msg1 = await botinitsk.send(
                            f'{feedback_debug} {feedback_automsg} Hi {taglist}you have not registered your attendance yet. <:peeposad:702156649992945674> Next time, {feedback_noangrypingplz}')
                else:
                    for discordtag in ping_tags:
                        taglist += '<@' + str(discordtag) + '>, '
                    if taglist != '':
                        msg1 = await botinitc.send(
                            f'{feedback_automsg} Hi {taglist}you have not registered your attendance yet. <:peeposad:702156649992945674> Next time, {feedback_noangrypingplz}')
                datasheet.update_cell(2, 9, str(msg1.id))  # save as string to avoid Excel nr truncation
                if debugger: await botinitsk.send(f'{feedback_debug} msg1 ID saved: `{msg1.id}`')
            except Exception as e:
                await botinitsk.send(f'Error: `{e}`')
            if debugger: await botinitsk.send(
                f'{feedback_debug} {ph_time_unformated.strftime("%H:%M:%S.%f:%A")} Angrypinger2 isreminded2={isreminded2} END')
            continue
            ...
            isreminded2 = False
            if debugger: await botinitsk.send(
                f'{feedback_debug} `[Timed event status reset] isarchived={isarchived} isreminded1={isreminded1} isreminded2={isreminded2}`')
            # Default timed event runtime (format = "%H:%M:%A")
            tf_archive = [t_silk2_archive, t_silk4_archive]
            tf_remind1 = [t_silk4_archive]
            tf_remind2 = ['22:00:Wednesday', '22:00:Friday']
            tf_reset = ['00:05:Sunday', '00:05:Monday', '22:05:Wednesday']
            if debugger: await botinitsk.send(
                f'{feedback_debug} `[Timed event timers reset] tf_archive={tf_archive} tf_remind1={tf_remind1} tf_remind2={tf_remind2} tf_reset={tf_reset}`')
            continue
            ...
            if no_of_args == 2:
                try:
                    ...
                    if not istimedeventformat(eventtime):
                        await ctx.send(
                            f'{feedback_properplz} Time format should be in H:M:Day, e.g. `/forcetimedevent, remind1, 22:00:Tuesday`')
                        return
                    ...
                    if eventname in answer_timedevent_archive:
                        ...
                        await ctx.send(
                            f'Timed event archive added to also run at {eventtime}. All schedules: {tf_archive}')
                    elif eventname in answer_timedevent_remind1:
                        global isreminded1
                        isreminded1 = False
                        tf_remind1.append(eventtime)
                        await ctx.send(
                            f'Timed event remind1 added to also run at {eventtime}. All schedules: {tf_remind1}')
                    elif eventname in answer_timedevent_remind2:
                        global isreminded2
                        isreminded2 = False
                        tf_remind2.append(eventtime)
                        await ctx.send(
                            f'Timed event remind2 added to also run at {eventtime}. All schedules: {tf_remind2}')
...
e.g. `/forcetimedevent, remind1, 22:00:Tuesday`''')
                ...
                await ctx.send(
                    f'{feedback_properplz} `/forcetimedevent, <name>, <time>`, e.g. `/forcetimedevent, remind1, 22:00:Tuesday`')
                return
        else:
            await ctx.send(f'*Nice try pleb.*')
    else:
...
async def jytest(ctx):
    ...
    try:
        ...
        global isremindenabled
        await ctx.send(
            f'isarchived={isarchived}, isreminded1={isreminded1}, isreminded2={isreminded2}, isremindenabled={isremindenabled}')
